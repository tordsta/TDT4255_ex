...
class CyclicDotSpec extends FlatSpec with Matchers {
  ...
  it should "Only signal valid output at end of calculation" in {
    ...
    iotesters.Driver.execute(() => new CyclicDot(3, 32), new TesterOptionsManager) { c =>
      IoSpec[CyclicDot](ins, c).myTester
    } should be(true)
  }
  ...
  it should "Be able to calculate dot products" in {
    ...
    def createProblem(vecLen: Int): List[CycleTask[CyclicDot]] = {
    ...
    def createProblems(vecLen: Int): List[CycleTask[CyclicDot]] =
    ...
    iotesters.Driver.execute(() => new CyclicDot(3, 32), new TesterOptionsManager) { c =>
      IoSpec[CyclicDot](createProblems(3), c).myTester
    } should be(true)
  }
}
