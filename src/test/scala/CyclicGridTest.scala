...
class CyclicGridSpec extends FlatSpec with Matchers {
  ...
  def writeRowCheck(dims: Dims, rowSel: Int => Int): Seq[CycleTask[CyclicGrid]] = {
    (0 until dims.cols).map( n =>
      CycleTask[CyclicGrid](
        n,
  ...
  it should "work like a regular CyclicVec when row select is fixed to 0" in {
    ...
    iotesters.Driver.execute(() => new CyclicGrid(dims, 32), new TesterOptionsManager) { c =>
      IoSpec[CyclicGrid](writeRowCheck(dims, _ => 0), c).myTester
    } should be(true)
  }
  ...
  it should "work like a regular CyclicVec when row select is fixed to 1" in {
    iotesters.Driver.execute(() => new CyclicGrid(dims, 32), new TesterOptionsManager) { c =>
      IoSpec[CyclicGrid](writeRowCheck(dims, _ => 1), c).myTester
    } should be(true)
  }
  ...
  it should "be able to write a matrix and output it" in {
    iotesters.Driver.execute(() => new CyclicGrid(dims, 32), new TesterOptionsManager) { c =>
      ...
      def writeMatrix(matrix: Matrix): List[CycleTask[CyclicGrid]] = {
      ...
      def readMatrix(matrix: Matrix): List[CycleTask[CyclicGrid]] = {
        (0 until dims.elements).toList.zipWithIndex.map{ case(n, idx) =>
          val row = n / dims.cols
          CycleTask[CyclicGrid](
            n,
      ...
      IoSpec[CyclicGrid](ins, c).myTester
    } should be(true)
  }
}
