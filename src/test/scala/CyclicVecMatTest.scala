...
class CyclicVecMatSpec extends FlatSpec with Matchers {

  def generateProblem(dims: Dims): List[CycleTask[CyclicVecMat]] = {
  ...
  it should "work" in {
    iotesters.Driver.execute(() => new CyclicVecMat(dims, 32), new TesterOptionsManager) { c =>
      IoSpec[CyclicVecMat](generateProblem(Dims(rows = 3, cols = 2)), c).myTester
    } should be(true)
  }
}
